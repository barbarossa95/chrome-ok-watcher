...
function getPostFromStorageOrCreate(url) {
...
function sendMessageViaTg(post) {
    ...
    if (post.hasLink || post.isNotified) {
        return;
    }
...
function formMessage(url, messageTemplate) {
    ...
    var mes = messageTemplate.replace(/{{LINK}}/g, link);
...
function setExtensionIcon() {
    ...
    var icon = {
        path    : 'images/ok-off.png',
    };
    var title = {
        title   : 'Ok-watcher stoped'
    };
    if (isActive) {
        icon["path"]="images/ok.png";
        title['title'] = 'Ok-watcher is running';
    }
